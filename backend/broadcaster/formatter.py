...
def format_alert_telegram(alert: dict) -> str:
    ...
    anomaly_label = html.escape(ANOMALY_LABELS.get(
        alert["anomaly_type"], alert["anomaly_type"]))
    contract = alert["contract_address"]
    contract_short = f"{contract[:8]}...{contract[-6:]}" if len(
        contract) > 14 else contract
    contract_link = f"{base}/address/{contract}"
    reason = html.escape(alert["reason"][:200])

    message = (
        f"{emoji} <b>Zeham Alert — {alert['severity']}</b>\n\n"
...
def format_alert_discord(alert: dict) -> dict:
    ...
    base = _explorer_base()
    emoji = SEVERITY_EMOJI.get(alert["severity"], "⚪")
    anomaly_label = ANOMALY_LABELS.get(
        alert["anomaly_type"], alert["anomaly_type"])
    contract = alert["contract_address"]

    fields = [
        {"name": "Anomaly Type",     "value": anomaly_label,
            "inline": True},
        {"name": "Severity",
            "value": f"{emoji} {alert['severity']}",       "inline": True},
        {"name": "Confidence",       "value": _confidence_pct(
            alert),               "inline": True},
        {"name": "Contract",
            "value": f"[`{contract[:20]}...`]({base}/address/{contract})", "inline": False},
        {"name": "Reason",
            "value": alert["reason"][:200],                "inline": False},
        {"name": "Detection Source", "value": _source_label(
            alert, short=True),     "inline": True},
    ]

    if alert.get("recommended_action"):
        fields.append({"name": "Recommended Action",
                      "value": alert["recommended_action"][:120], "inline": False})
    ...
    return {
        "embeds": [{
            "title":       f"{emoji} Zeham Security Alert",
            ...
            "fields":      fields,
            "footer":      {"text": f"Zeham v1.0.0 · Mantle Network · Alert ID: {str(alert['id'])[:8]}"},
            "timestamp":   timestamp,
        }]
    }
